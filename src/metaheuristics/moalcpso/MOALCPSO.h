...
class MOALCPSO : public Algorithm {
...
private:
  ...
  //double velocityConstriction(double v, double * deltaMax,
   ...
   //                                     int variableIndex,
   //                                     int particleIndex);
  void computeSpeed(int iter, int miter, int age_iter);
  void computeNewPositions();
  void moalcpsoMutation(int actualIteration, int totalIterations);
  void initParams();
  void setAlgParams();
  void deleteParams();
  
  void generateChallenger();
  void evaluateChallenger(int iteration_, int maxIterations_);

public:
  MOALCPSO(Problem * problem);
